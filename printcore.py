...
class printcore():
    def __init__(self, host = None, port = None):
        ...
        self.loud = True #emit sent and received lines to terminal wangly
    ...
    def connect(self, host = None, port = None):
        ...
        if self.host is not None and self.port is not None:
            ...
            print "\n"
            print self.port
            self.printer = telnetlib.Telnet(self.host, self.port, 0.5)
#            self.printer.set_debuglevel(2)  
            self.printer.read_until("password:")
            self.printer.write("reprap\n")
            self.printer.read_until("Log in successful!")
            self.stop_read_thread = False
    ...
    def _listen_until_online(self):
        while not self.online and self._listen_can_continue():
            ...
            while self._listen_can_continue():
                ...
                if line.startswith(tuple(self.greetings)) or line.startswith('T:'):
                    if self.onlinecb:
                        try: self.onlinecb()
                        except: pass
                    self.online = True
