...
#        sys.stdout.write(self.prompt)
    ...
    def help_autobedlevel(self):
        print "Automatic calebrate the bed level."
    ...
    def waitforok(self, line):
        if "fits points" in line:
            print "Received OK."
            self.ok = True
            

    def do_waitforok(self):
        print "Start wait for ok"
        self.recvlisteners+=[self.waitforok]

        print self.recvlisteners
        while not self.ok:
            continue
        print "Received OK"
        self.ok = False
        self.recvlisteners.remove(self.waitforok)

    def do_waitautolevel(self):
        self.p.printer.read_until("fits points", 150)
        self.p.printer.read_until("\r\n")
        print "Auto leveling finished"
...
if __name__ == "__main__":
    ...
    interp = pronsole()
    interp.do_connect("192.168.1.14 23")
    time.sleep(2)

    interp.do_home("")
    time.sleep(10)
#    interp.do_waitforok()
    interp.do_autobedlevel()
    interp.do_waitforok()
    time.sleep(1)
    interp.do_sdprint("snowman.g")
    time.sleep(10)
    interp.do_monitor("10")
